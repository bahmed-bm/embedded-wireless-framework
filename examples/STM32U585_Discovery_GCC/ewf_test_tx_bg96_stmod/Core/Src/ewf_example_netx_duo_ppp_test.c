/*
 * ewf_example_netx_duo_ppp_test.c
 *
...
#warning "TODO: fix the MACRO definition or include order, which causes the NX_IP to be defined with different sizes!"
#include "tx_api.h"
...
#if(NX_ENABLE_TCPIP_OFFLOAD)
#include "nx_ip.h"
//#include "../../Middlewares/ST/netxduo/common/src/nx_tcp_socket_driver_packet_receive.c"
//#include "../../Middlewares/ST/netxduo/common/src/nx_udp_socket_driver_packet_receive.c"
...
ewf_result ewf_example_netx_duo_ppp_test(ewf_adapter* adapter_ptr)
{
    ...
    if (ewf_result_failed(result))
    {
        EWF_LOG_ERROR("Failed to get the adapter IPv4 DNS: ewf_result %d. Using 8.8.8.8 as an alternative\n", result);
        dns_address = IP_ADDRESS(8, 8, 8, 8);
        //return EWF_RESULT_NOT_SUPPORTED;
    }
    ...
    if(pool_0.nx_packet_pool_id == 0 && pool_0.nx_packet_pool_size == 0)
    {
    ...
    {
    	status = NX_SUCCESS; // pool already exists
    }
    ...
    {
    	status = NX_SUCCESS; // pool already created
    }
    ...
    ewf_platform_sleep(EWF_PLATFORM_TICKS_PER_SECOND*2);
...
{
#if(NX_ENABLE_TCPIP_OFFLOAD)
	EWF_LOG("ICMP ping not supported when NX_ENABLE_TCPIP_OFFLOAD is enabled!\n");
#else
        ...
        else
        {
            ...
            else
            {
                ...
                else
                {
                    ...
                    status = nx_packet_allocate(&pool_0, &packet_ptr, 2*NX_TCP_PACKET, NX_WAIT_FOREVER);
                    if (status)
                    {
                        EWF_LOG_ERROR("Failed to allocate the packet\n");
                    }
