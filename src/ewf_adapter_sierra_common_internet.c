 ...
 * brief The Embedded Wireless Framework Sierra common Internet(TCP+UDP) implementation
 ****************************************************************************/
...
ewf_adapter_api_tcp ewf_adapter_sierra_common_api_tcp =
{
    ewf_adapter_sierra_common_tcp_open,
    ewf_adapter_sierra_common_tcp_close,

    ewf_adapter_sierra_common_tcp_control,

    ewf_adapter_sierra_common_tcp_set_tls_configuration,

    ewf_adapter_sierra_common_tcp_bind,
    ewf_adapter_sierra_common_tcp_listen,
    ewf_adapter_sierra_common_tcp_accept,
    ewf_adapter_sierra_common_tcp_connect,
    ewf_adapter_sierra_common_tcp_shutdown,
    ewf_adapter_sierra_common_tcp_send,
    ewf_adapter_sierra_common_tcp_receive,
};

ewf_adapter_api_udp ewf_adapter_sierra_common_api_udp =
{
    ewf_adapter_sierra_common_udp_open,
    ewf_adapter_sierra_common_udp_close,

    ewf_adapter_sierra_common_udp_control,

    ewf_adapter_sierra_common_udp_set_dtls_configuration,

    ewf_adapter_sierra_common_udp_bind,
    ewf_adapter_sierra_common_udp_shutdown,
    ewf_adapter_sierra_common_udp_send_to,
 ...
 ******************************************************************************/
  ...
  * @return an index to an available entry in the socket pool, or EWF_ADAPTER_SIERRA_COMMON_INTERNET_SOCKET_INVALID if not available
  */
static ewf_result _ewf_adapter_sierra_common_internet_socket_pool_allocate(ewf_adapter* adapter_ptr, uint32_t * socket_id_ptr)
{
    EWF_ADAPTER_VALIDATE_POINTER(adapter_ptr);
    ewf_adapter_sierra_common* adapter_sierra_common_ptr = (ewf_adapter_sierra_common*)adapter_ptr->implementation_ptr;

    if (socket_id_ptr == NULL) return EWF_RESULT_INVALID_FUNCTION_ARGUMENT;

    *socket_id_ptr = (uint32_t)EWF_ADAPTER_SIERRA_COMMON_INTERNET_SOCKET_INVALID;

    for (int i = 0; i < EWF_ADAPTER_SIERRA_COMMON_INTERNET_SOCKET_POOL_SIZE; ++i)
    {
        if (adapter_sierra_common_ptr->internet_socket_pool[i].used == false)
        {
...
ewf_result ewf_adapter_sierra_common_internet_start(ewf_adapter* adapter_ptr)
{
    ...
    uint32_t i;
    for (i = 0; i < EWF_ADAPTER_SIERRA_COMMON_INTERNET_SOCKET_POOL_SIZE; ++i)
    {
        adapter_sierra_common_ptr->internet_socket_pool[i].socket_ptr = NULL;
        adapter_sierra_common_ptr->internet_socket_pool[i].id = i+1; // connId starts from 1 for RYZ014
        adapter_sierra_common_ptr->internet_socket_pool[i].local_port = 0;
        adapter_sierra_common_ptr->internet_socket_pool[i].type = ewf_adapter_sierra_common_socket_service_type_not_initialized;
        adapter_sierra_common_ptr->internet_socket_pool[i].used = false;
        adapter_sierra_common_ptr->internet_socket_pool[i].conn = false;
        adapter_sierra_common_ptr->internet_socket_pool[i].conn_error = false;
        adapter_sierra_common_ptr->internet_socket_pool[i].recv = false;
    }
    ...
    if (ewfl_str_starts_with((char*)buffer_ptr, urc_prefix_str))
    {
        char* urc_str = (char*) buffer_ptr + ewfl_str_length(urc_prefix_str);
        {
            ...
            if (ewfl_str_starts_with(urc_str, match_str))
            {
                ...
                if(!(socket_connect_id)|| (EWF_ADAPTER_SIERRA_COMMON_INTERNET_SOCKET_POOL_SIZE < socket_connect_id))
                {
                ...
                {
                    ewf_adapter_sierra_common_internet_socket* internet_socket_ptr = &(adapter_sierra_common_ptr->internet_socket_pool[socket_connect_id - 1]);
                    if (internet_socket_ptr->socket_ptr)
                    {
                        switch (internet_socket_ptr->type)
                        {
                        case ewf_adapter_sierra_common_socket_service_type_tcp:
                        // fall through
                        // no break
                        case ewf_adapter_sierra_common_socket_service_type_tcp_listener:
                        {
                        ...
                        break;
                        case ewf_adapter_sierra_common_socket_service_type_udp:
                        // fall through
                        // no break
                        case ewf_adapter_sierra_common_socket_service_type_udp_listener:
                        {
                            internet_socket_ptr->recv = true;
                            internet_socket_ptr->conn = true;
                            internet_socket_ptr->conn_error = false;
        ...
        }
        {
            ...
            if (ewfl_str_starts_with(urc_str, match_str))
            {
                char* parse_str = urc_str + ewfl_str_length(match_str);
                uint32_t socket_connect_id = (uint32_t)atoi(parse_str);
                adapter_sierra_common_ptr->internet_socket_pool[socket_connect_id - 1].conn = false;
                adapter_sierra_common_ptr->internet_socket_pool[socket_connect_id - 1].conn_error = true;
                ewf_adapter_sierra_common_internet_socket* internet_socket_ptr = &(adapter_sierra_common_ptr->internet_socket_pool[socket_connect_id - 1]);
...
static ewf_result _ewf_adapter_sierra_common_internet_socket_open(ewf_adapter* adapter_ptr, ewf_adapter_sierra_common_internet_socket* internet_socket_ptr, uint32_t service_type, uint32_t udp_local_port, char const* server_str, uint32_t remote_port, uint32_t accept_any_remote)
{
    EWF_ADAPTER_VALIDATE_POINTER(adapter_ptr);
...
static ewf_result _ewf_adapter_sierra_common_internet_socket_close(ewf_adapter* adapter_ptr, ewf_adapter_sierra_common_internet_socket* internet_socket_ptr)
{
    EWF_ADAPTER_VALIDATE_POINTER(adapter_ptr);
    ewf_interface* interface_ptr = adapter_ptr->interface_ptr;
    EWF_INTERFACE_VALIDATE_POINTER(interface_ptr);
...
static ewf_result _ewf_adapter_sierra_common_internet_socket_send(ewf_adapter* adapter_ptr, ewf_adapter_sierra_common_internet_socket* internet_socket_ptr, const char* remote_address_str, uint32_t remote_port, const uint8_t* buffer_ptr, uint32_t buffer_length)
{
    EWF_ADAPTER_VALIDATE_POINTER(adapter_ptr);
    ewf_interface* interface_ptr = adapter_ptr->interface_ptr;
    EWF_INTERFACE_VALIDATE_POINTER(interface_ptr);
    ...
    if (internet_socket_ptr->type == ewf_adapter_sierra_common_socket_service_type_udp ||
        internet_socket_ptr->type == ewf_adapter_sierra_common_socket_service_type_udp_listener)
    {
    ...
    else if (internet_socket_ptr->type == ewf_adapter_sierra_common_socket_service_type_tcp ||
             internet_socket_ptr->type == ewf_adapter_sierra_common_socket_service_type_tcp_listener)
    {
        if (remote_address_str != NULL || remote_port != 0)
        {
    ...
    {
        if ((internet_socket_ptr->type == ewf_adapter_sierra_common_socket_service_type_udp)||(internet_socket_ptr->type == ewf_adapter_sierra_common_socket_service_type_udp_listener))
        {
            result = _ewf_adapter_sierra_common_internet_socket_open(adapter_ptr, internet_socket_ptr,
                                                                      EWF_ADAPTER_SIERRA_COMMON_INTERNET_SOCKET_UDP,
    ...
    buffer_length = (buffer_length > EWF_ADAPTER_SIERRA_COMMON_SOCKET_MAX_SEND_SIZE) ? EWF_ADAPTER_SIERRA_COMMON_SOCKET_MAX_SEND_SIZE : buffer_length;
...
struct _ewf_adapter_sierra_common_sqnsrecv_message_tokenizer_pattern_match_function_state
{
...
static bool _ewf_adapter_sierra_common_sqnsrecv_message_tokenizer_pattern_match_function(const uint8_t* buffer_ptr, uint32_t buffer_length, const ewf_tokenizer_basic_pattern* pattern_ptr, bool* stop_ptr)
{
    ...
    struct _ewf_adapter_sierra_common_sqnsrecv_message_tokenizer_pattern_match_function_state* state_ptr =
...
static ewf_result _ewf_adapter_sierra_common_internet_socket_receive(
    ewf_adapter* adapter_ptr,
    ewf_adapter_sierra_common_internet_socket* internet_socket_ptr,
    ...
    bool wait)
{
    ...
    read_length = (read_length < EWF_ADAPTER_SIERRA_COMMON_SOCKET_MAX_RECEIVE_SIZE) ? (read_length) : (EWF_ADAPTER_SIERRA_COMMON_SOCKET_MAX_RECEIVE_SIZE);
...
ewf_result ewf_adapter_sierra_common_tcp_open(ewf_adapter* adapter_ptr, ewf_socket_tcp* socket_ptr)
{
    ...
    if (ewf_result_failed(result = _ewf_adapter_sierra_common_internet_socket_pool_allocate(adapter_ptr, &socket_id)))
    {
        EWF_LOG_ERROR("Too many open sockets.");
        return EWF_RESULT_TOO_MANY_OPEN_SOCKETS;
    }

    ewf_adapter_sierra_common_internet_socket* internet_socket_ptr = &(adapter_sierra_common_ptr->internet_socket_pool[socket_id]);

    socket_ptr->data_ptr = internet_socket_ptr;
    internet_socket_ptr->socket_ptr = socket_ptr;
    internet_socket_ptr->type = ewf_adapter_sierra_common_socket_service_type_tcp;
...
ewf_result ewf_adapter_sierra_common_tcp_close(ewf_socket_tcp* socket_ptr)
{
    ...
    EWF_ADAPTER_VALIDATE_POINTER(adapter_ptr);
    ewf_adapter_sierra_common_internet_socket* internet_socket_ptr = (ewf_adapter_sierra_common_internet_socket*)socket_ptr->data_ptr;
    return _ewf_adapter_sierra_common_internet_socket_close(adapter_ptr, internet_socket_ptr);
}
...
ewf_result ewf_adapter_sierra_common_tcp_listen(ewf_socket_tcp* socket_ptr)
{
    ...
    if (ewf_result_failed(result = ewf_interface_send_commands(interface_ptr, "AT+SQNSL=", socket_cstr, ",",EWF_ADAPTER_SIERRA_COMMON_SOCKET_COMMAND_MODE,",", port_cstr, "\r", NULL)))
        return result;
    if (ewf_result_failed(result = ewf_interface_verify_response(interface_ptr, "\r\nOK\r\n")))
        return result;

    adapter_sierra_common_ptr->internet_socket_pool[internet_socket_ptr->id - 1].used = true;
    adapter_sierra_common_ptr->internet_socket_pool[internet_socket_ptr->id - 1].open_error = false;
...
ewf_result ewf_adapter_sierra_common_tcp_accept(ewf_socket_tcp* socket_ptr,  ewf_socket_tcp* socket_new_ptr)
{
    ...
    if ((internet_socket_ptr->id == 0) || (EWF_ADAPTER_SIERRA_COMMON_INTERNET_SOCKET_POOL_SIZE < internet_socket_ptr->id))
    {
    ...
    if (true)
    {
        ...
        for (;;)
        {
            if (adapter_sierra_common_ptr->internet_socket_pool[internet_socket_ptr->id - 1].recv)
            {
                adapter_sierra_common_ptr->internet_socket_pool[internet_socket_ptr->id - 1].recv = false;
                break;
            }
            ewf_interface_poll(interface_ptr);
            ewf_platform_sleep(1);
    ...
    {
        ewf_interface_poll(interface_ptr);
        if (adapter_sierra_common_ptr->internet_socket_pool[internet_socket_ptr->id - 1].recv == false)
        {
...
ewf_result ewf_adapter_sierra_common_tcp_connect(ewf_socket_tcp * socket_ptr, const char* const server_str, uint32_t remote_port)
{
    EWF_VALIDATE_TCP_SOCKET_POINTER(socket_ptr);
    ewf_adapter* adapter_ptr = socket_ptr->adapter_ptr;
    ewf_adapter_sierra_common_internet_socket* internet_socket_ptr = (ewf_adapter_sierra_common_internet_socket*)socket_ptr->data_ptr;
    return _ewf_adapter_sierra_common_internet_socket_open(adapter_ptr, internet_socket_ptr,
                                                            EWF_ADAPTER_SIERRA_COMMON_INTERNET_SOCKET_TCP,
...
ewf_result ewf_adapter_sierra_common_tcp_send(ewf_socket_tcp* socket_ptr, const uint8_t* buffer_ptr, uint32_t buffer_length)
{
    ...
    {
        _ewf_adapter_sierra_common_internet_socket_close(adapter_ptr, internet_socket_ptr);
        EWF_LOG_ERROR("Socket is closed by remote.");
        return EWF_RESULT_CONNECTION_FAILED;

    }
    return _ewf_adapter_sierra_common_internet_socket_send(adapter_ptr, internet_socket_ptr,
                                                            NULL, 0, /* Do not specify the remote address and port, they are given by the connection */
                                                            buffer_ptr, buffer_length);

}

ewf_result ewf_adapter_sierra_common_tcp_receive(ewf_socket_tcp* socket_ptr, uint8_t* buffer_ptr, uint32_t* buffer_length_ptr, bool wait)
{
    EWF_VALIDATE_TCP_SOCKET_POINTER(socket_ptr);
    ewf_adapter* adapter_ptr = socket_ptr->adapter_ptr;
    EWF_ADAPTER_VALIDATE_POINTER(adapter_ptr);
    ...
    return _ewf_adapter_sierra_common_internet_socket_receive(
        adapter_ptr,
        internet_socket_ptr,
...
ewf_result ewf_adapter_sierra_common_udp_open(ewf_adapter* adapter_ptr, ewf_socket_udp* socket_ptr)
{
    ...
    socket_ptr->data_ptr = &(adapter_sierra_common_ptr->internet_socket_pool[socket_id]);
    ewf_adapter_sierra_common_internet_socket* internet_socket_ptr = &(adapter_sierra_common_ptr->internet_socket_pool[socket_id]);

    socket_ptr->data_ptr = internet_socket_ptr;
    internet_socket_ptr->socket_ptr = socket_ptr;
    internet_socket_ptr->type = ewf_adapter_sierra_common_socket_service_type_udp;
    internet_socket_ptr->used = true;
...
ewf_result ewf_adapter_sierra_common_udp_set_dtls_configuration(ewf_socket_udp* socket_ptr, uint32_t tls_configuration_id)
{
    EWF_VALIDATE_UDP_SOCKET_POINTER(socket_ptr);
    ewf_adapter* adapter_ptr = socket_ptr->adapter_ptr;
    EWF_ADAPTER_VALIDATE_POINTER(adapter_ptr);
...
ewf_result ewf_adapter_sierra_common_udp_send_to(ewf_socket_udp* socket_ptr, const char* remote_address_str, uint32_t remote_port, const uint8_t* buffer_ptr, uint32_t buffer_length)
{
    ...
    return _ewf_adapter_sierra_common_internet_socket_send(
        adapter_ptr,
        internet_socket_ptr,
