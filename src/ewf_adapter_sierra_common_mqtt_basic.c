 ...
 * @brief The Embedded Wireless Framework Common Sierra MQTT basic API implementation
 ****************************************************************************/
...
ewf_adapter_api_mqtt_basic ewf_adapter_sierra_common_api_mqtt_basic =
{
    ewf_adapter_sierra_common_mqtt_basic_default_state_callback,
    ewf_adapter_sierra_common_mqtt_basic_default_message_callback,

    ewf_adapter_sierra_common_mqtt_basic_urc_callback,

    NULL, /* control, not implemented */

    ewf_adapter_sierra_common_mqtt_basic_connect,
    ewf_adapter_sierra_common_mqtt_basic_disconnect,
    ewf_adapter_sierra_common_mqtt_basic_subscribe,
    ewf_adapter_sierra_common_mqtt_basic_unsubscribe,
    ewf_adapter_sierra_common_mqtt_basic_publish,
    ewf_adapter_sierra_common_mqtt_basic_message_get,
};
    ...
    if (ewfl_str_starts_with((char*)buffer_ptr, urc_prefix_str))
    {
...

        {
            ...
            if (ewfl_str_starts_with(urc_str, match_str))
            {
                ...
                if(err == 0)
                {
                    adapter_sierra_common_ptr->mqtt_basic_conn = true;
                    adapter_sierra_common_ptr->mqtt_basic_conn_error =  (uint8_t)err;
                    return EWF_RESULT_OK;
                }
                else
                {
                    adapter_sierra_common_ptr->mqtt_basic_conn = false;
                    adapter_sierra_common_ptr->mqtt_basic_conn_error = (uint8_t)err;
...

        {
            ...
            if (ewfl_str_starts_with(urc_str, match_str))
            {
                ...
                if(err != 0)
                {
                    ...
                    adapter_sierra_common_ptr->mqtt_basic_publish_error = (uint8_t)err;
                }
                adapter_sierra_common_ptr->mqtt_basic_publish = true;
                return EWF_RESULT_OK;
            }
        }

        {
            ...
            if (ewfl_str_starts_with(urc_str, match_str))
            {
                ...
                if(err != 0)
                {
                    ...
                    adapter_sierra_common_ptr->mqtt_basic_subscribe_error = (uint8_t)err;
                }
                adapter_sierra_common_ptr->mqtt_basic_subscribe = true;
                return EWF_RESULT_OK;
            }
        }

        {
            ...
            if (ewfl_str_starts_with(urc_str, match_str))
            {
                ...
                {
                    adapter_sierra_common_ptr->mqtt_basic_conn = false;
                    adapter_sierra_common_ptr->mqtt_basic_conn_error = false;
                    return EWF_RESULT_OK;
                }
                else
                {
                    adapter_sierra_common_ptr->mqtt_basic_conn = false;
                    adapter_sierra_common_ptr->mqtt_basic_conn_error = true;
...
ewf_result ewf_adapter_sierra_common_mqtt_basic_start(ewf_adapter* adapter_ptr)
{
    ...
    ewf_adapter_sierra_common* implementation_ptr = (ewf_adapter_sierra_common*)adapter_ptr->implementation_ptr;
...
ewf_result ewf_adapter_sierra_common_mqtt_basic_connect(ewf_adapter* adapter_ptr, char const* const server, uint32_t port, char const* const clientid, char const* const username, char const* const password)
{
    EWF_ADAPTER_VALIDATE_POINTER(adapter_ptr);
    ewf_adapter_sierra_common* adapter_sierra_common_ptr = (ewf_adapter_sierra_common*)adapter_ptr->implementation_ptr;
    ...
    {
        if (ewf_result_failed(result = ewf_interface_send_commands(interface_ptr, "AT+SQNSMQTTCFG=0,\"", clientid, "\",\"", username, "\",,", EWF_CONFIG_ADAPTER_SIERRA_COMMON_SECURITY_PROFILE_ID, "\r", NULL))) return result;
    }
    else /* username and password provided */
    {
        if (ewf_result_failed(result = ewf_interface_send_commands(interface_ptr, "AT+SQNSMQTTCFG=0,\"", clientid, "\",\"", username, "\",\"", password, "\",", EWF_CONFIG_ADAPTER_SIERRA_COMMON_SECURITY_PROFILE_ID, "\"\r", NULL))) return result;
    }
    ...
    {
        if (adapter_sierra_common_ptr->mqtt_basic_conn) break;
        if (adapter_sierra_common_ptr->mqtt_basic_conn_error) return EWF_RESULT_UNEXPECTED_RESPONSE;
...
ewf_result ewf_adapter_sierra_common_mqtt_basic_disconnect(ewf_adapter* adapter_ptr)
{
    ...
    {
        if (adapter_sierra_common_ptr->mqtt_basic_conn == false) break;
        ewf_interface_poll(interface_ptr);
        ewf_platform_sleep(1);
    }
...
ewf_result ewf_adapter_sierra_common_mqtt_basic_subscribe(ewf_adapter* adapter_ptr, char const* topic)
{
    ...
    if(adapter_sierra_common_ptr->mqtt_basic_conn_error) return EWF_RESULT_ADAPTER_MQTT_DISCONNECTED;
    ...
    {
        if (adapter_sierra_common_ptr->mqtt_basic_subscribe) break;
        ewf_interface_poll(interface_ptr);
        ewf_platform_sleep(1);
    }
    adapter_sierra_common_ptr->mqtt_basic_subscribe = false;
    if(adapter_sierra_common_ptr->mqtt_basic_subscribe_error)
    {
...
ewf_result ewf_adapter_sierra_common_mqtt_basic_publish(ewf_adapter* adapter_ptr, const char* const topic, const char* message_cstr)
{
    EWF_ADAPTER_VALIDATE_POINTER(adapter_ptr);
    ...
    for (unsigned i = 0; i < (2 * EWF_PLATFORM_TICKS_PER_SECOND); i++)
    {
        if (adapter_sierra_common_ptr->mqtt_basic_publish) break;
        ewf_interface_poll(interface_ptr);
        ewf_platform_sleep(1);
    }
    adapter_sierra_common_ptr->mqtt_basic_publish = false;
    if(adapter_sierra_common_ptr->mqtt_basic_publish_error)
    {
...
ewf_result ewf_adapter_sierra_common_mqtt_basic_message_get(ewf_adapter* adapter_ptr, const char* topic_cstr, char* message_buffer_ptr)
{
    EWF_ADAPTER_VALIDATE_POINTER(adapter_ptr);
