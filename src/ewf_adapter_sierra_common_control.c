 ...
 * brief The Embedded Wireless Framework Sierra common adapter control driver
 ****************************************************************************/
...
ewf_adapter_api_control ewf_adapter_sierra_common_api_control =
{
    ewf_adapter_sierra_common_start,
    ewf_adapter_sierra_common_stop,
};
...
ewf_result ewf_adapter_sierra_common_start(ewf_adapter* adapter_ptr)
{
    ...
    result = ewf_adapter_sierra_common_tokenizer_init(interface_ptr);
    ...
    if (ewf_result_failed(result = ewf_adapter_sierra_common_internet_start(adapter_ptr)))
    {
    ...
    if (ewf_result_failed(result = ewf_adapter_sierra_common_tls_basic_start(adapter_ptr)))
    {
    ...
    if (ewf_result_failed(result = ewf_adapter_sierra_common_mqtt_basic_start(adapter_ptr)))
    {
        EWF_LOG_ERROR("Failed to start the MQTT basic API: ewf_result %d.\n", result);
        return EWF_RESULT_INTERFACE_INITIALIZATION_FAILED;
    }
#endif
    adapter_sierra_common_ptr->module_start = false;
...
#if EWF_ADAPTER_SIERRA_COMMON_NVM_ENABLED
    if (ewf_result_failed(result = ewf_adapter_sierra_common_nvm_stop(adapter_ptr)))
    {
...
#if defined(EWF_ADAPTER_SIERRA_COMMON_TCP_ENABLED) || defined(EWF_ADAPTER_SIERRA_COMMON_UDP_ENABLED)
    if (ewf_result_failed(result = ewf_adapter_sierra_common_internet_stop(adapter_ptr)))
    {
...
#if EWF_ADAPTER_SIERRA_COMMON_TLS_BASIC_ENABLED
    if (ewf_result_failed(result = ewf_adapter_sierra_common_tls_basic_stop(adapter_ptr)))
    {
...
#if EWF_ADAPTER_SIERRA_COMMON_MQTT_BASIC_ENABLED
    if (ewf_result_failed(result = ewf_adapter_sierra_common_mqtt_basic_stop(adapter_ptr)))
    {
