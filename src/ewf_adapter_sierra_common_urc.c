...
ewf_result ewf_adapter_sierra_common_urc_callback(ewf_interface* interface_ptr, uint8_t* buffer_ptr, uint32_t buffer_length)
{
    ...
    {
        adapter_sierra_common_ptr->module_start = true;
        return EWF_RESULT_OK;
    }
    ...
    if (ewf_result_failed(result = ewf_adapter_sierra_common_internet_urc_callback(interface_ptr, buffer_ptr, buffer_length))) return result;
    ...
    if (ewf_result_failed(result = ewf_adapter_sierra_common_mqtt_basic_urc_callback(interface_ptr, buffer_ptr, buffer_length))) return result;
    if (result != EWF_RESULT_UNHANDLED_URC) return EWF_RESULT_OK;
#endif
